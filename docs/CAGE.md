...
Install the project with `cmake --install`, provision the state directory with
`systemd-tmpfiles --create holonight-greeter.conf`, then merge the documented example into greetd configuration. The
tested launcher shape is:
...
restart greetd, and remove the installed greeter files if necessary. CMake installation never owns or modifies
