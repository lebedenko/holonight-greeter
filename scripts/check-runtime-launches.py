...
def main():
    ...
    for mode in ("default", "environment", "command-line", "external-config"):
        with tempfile.TemporaryDirectory(prefix="uqc106-") as directory:
            root = Path(directory)
            env = dict(PATH="", LANG="C.UTF-8", HOME=str(root),
                       ...
                       DBUS_SYSTEM_BUS_ADDRESS=f"unix:path={root}/unavailable-system-bus",
                       GREETD_SOCK=str(root / "unavailable-greetd"))
            for key in ("XDG_CONFIG_HOME", "XDG_DATA_HOME", "XDG_CACHE_HOME", "XDG_RUNTIME_DIR",
                        "XDG_CONFIG_DIRS", "XDG_DATA_DIRS"):
                path = root / key
                path.mkdir(mode=0o700)
                env[key] = str(path)
            command = [str(executable), "--demo", "--config", str(root / "greeter.toml"),
